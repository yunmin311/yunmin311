...
| `workSlots` | 03 页面上放几张卡，默认 6（改这个要连布局一起改，见第五节） |
...
- **`tier`** 决定它是不是必定出现：
  - `"core"` —— 永远展示。定义这个页面的作品放这里。
  - `"candidate"`（默认）—— 有资格竞争剩下的名额，按活跃度排。
- **`languages`** 决定它算不算进语言占比：
  - `"count"`（默认）—— 算。
  - `"exclude"` —— 是真实代码，但算进去会失真（生成物、vendored）。
  - `"n/a"` —— 没有可对比的「新增代码行」口径，比如纯样式表或提示词规范。
- **`weight`**（可选，0–1）是**下限**不是加分：把「最近没动但很重要」的项目托住，
  不让它被活跃度埋掉；但它**不可能**压过正在积极开发的项目。
- **`hysteresis`**（可选，天数）是挑战者要领先在位者多少天才换人。默认 0。
  给在位项目设 10–21，一次小 commit 就不会把整排卡洗一遍。
- `key` 是文件名。改了它，README 里那张图的路径要跟着改——**不想改路径就别动 `key`。**

**入选规则**（`scripts/lib/projects.mjs`）：`core` 先占位，剩下的名额给
`candidate` 按分数竞争，分数 = 0.6 × 最近度（距我最后一次提交的天数，180 天线性衰减）
+ 0.4 × 体量（我写的行数的 log10）。**刻意不用** star / fork / 裸 commit 数——
那些衡量的是观众，不是工作。
...
3. **作品卡跑偏了** — 报错长这样：
   ...
   意思是：有个项目比页面上某张卡活跃得多，但一直没被选上。通常是两种情况——
   新项目没加进 `projects`，或者在位那张卡的 `hysteresis` 设得太大把它焊死了。
   去 `projects` 里补一条，或者把那张卡的 `hysteresis` 调小。
...
>
> **例外，而且很重要**：上面第 3 条（作品卡跑偏）无论是谁触发的都会开 Issue。
> 因为它不是「接口抖了一下」，是规则本身和现实脱节了，等下一轮也不会好。
> 这也正是它被加进来的原因——以前这种情况什么都不会发生。

...
点 **Revert** 撤回。修好后机器人自己重跑，**那个 Issue 会自己关掉**。

> **什么时候会打扰你，什么时候不会。**
> 规则是「**同一种失败连续发生两次**（约 12 小时）」才开 Issue，与是谁触发的无关；
> 第一次失败永远安静——接口抖一下不值得半夜叫你。
> 你**自己改文件**触发的那次失败会立刻通知，因为那时你正盯着。
>
> 这条规则以前是另一套：定时跑挂了**永不**通知，理由是「六小时后自己就好了」。
> 2026-09-17 这个假设破产了——`METRICS_TOKEN` 被拒，连续 4 次定时失败、跨 17 小时，
> 凭据坏了不会自己好，而没有任何人被告知。所以现在看的是**重复**，不是触发方式。
>
> **读不到数据的时候不会失败，也不会画成空的。** 日志里会出现
> 「holding … the last known-good …」或「HELD — keeping the last good image」，
> 意思是用上一轮记下来的真实数据继续画。**绝不会**画成 0、空列表或换个猜测。
> 只有**从来没有成功过**（第一次构建就没有旧图可留）才会直接报错。
...
- **60 天休眠**：GitHub 规定，公开仓库的定时任务在「仓库连续 60 天没有活动」后
  会被自动停用。实测不必担心这个：机器人自己的提交**每天 3–7 次**，
  最近 24 天每一天都有（最长空档 1 天），而阈值是 60 天。
  万一天发现热力图不动了，先去 **Actions** 页面看 `Rebuild panels` 是不是被禁用了——
  被禁用会有一个 Enable workflow 的按钮，点一下就回来。
- **定时不是准时，但也不慢**：`17 */6 * * *` 是意图，不是保证；GitHub 明确说了排队任务
  在高负载时会延迟、极端情况下会被丢弃。实测最近 25 天 94 次定时运行：
  平均 **3.74 次/天**，间隔中位数 **6.5 小时**、90 分位 **8.5 小时**、最长 **13.1 小时**。
  这个散布是 GitHub 排队造成的，仓库这边控制不了。急着看到结果就手动 **Run workflow**
  —— 实测从点下到跑完约 **18 秒**。
- **刚跑完刷新可能还是旧图（最多 5 分钟）**：README 里的 SVG 走
  `github.com/<owner>/<repo>/raw/main/...`，302 到 `raw.githubusercontent.com`，
  那边的缓存是 `max-age=300`。所以重建完成后，图片最多还会旧 5 分钟。
  README 里的**文字**（包括图片的 alt）是立刻生效的，只有图片有这个尾巴。
  看到旧图先强刷一次，别急着当故障。
